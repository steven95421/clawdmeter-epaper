...
# Pure logic (stdlib-only, unit-tested in tests/test_logic.py) lives in a
# sibling module so it can be imported and tested without the BLE stack.
...
from usage_logic import extract_access_token as _extract_access_token
...
async def main() -> None:
    ...
    # (scan-scheduling constants + decide_scan_mode() live in usage_logic.py)
